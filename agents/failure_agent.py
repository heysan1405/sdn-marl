"""
DQN Failure Agent for SDN-MARL.
"""
...
from models.failure_network import FailureNetwork


class FailureAgent:
    """
    Agent responsible for network failure response.
    ...
    def __init__(
        ...
        self.online_network = FailureNetwork(
            graph_embedding_dim=graph_embedding_dim,
    ...
    def select_action(
        ...
        """
        Select a failure-response action.

        Restores the original training/evaluation mode
        ...
        # Temporarily use evaluation mode
    ...
    def remember(
        ...
        if not 0 <= action < self.NUM_ACTIONS:
            raise ValueError(
                f"Invalid failure action: {action}"
            )
